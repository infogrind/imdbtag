...
# Two lists for notifications in offline mode. The first is for notifications of
# renamings done, the second for unknown movies (where no IMDb match was found).
...
def main():
  ...
  # Now do the actual processing, depending on whether we are in directory mode
  # or not.
  if dirmode:
    process_directory(directory)
  else:
    for b in args:
      # Need to strip trailing slash of directory names, otherwise dirname and
      # basename get confused.
      b = b.rstrip('/')
...
def process(b, f):
  ...
  # If there is a movie file that is not in its own directory, we ask the user
  # whether a directory should be made for the file. If yes, we continue by
  # processing the new directory; otherwise we skip the file.
...
def tag(b, d):
  ...
  debug('Verifying whether "' + os.path.join(b, d) + '" is a valid directory.')
  ...
  if recoverymode:
    if has_original_file(b, d):
      ...
      try:
        debug('Recovery mode: Clearing directory "' + d + '" and renaming ' +
            'it to "' + o + '".')
        clear_directory(b, d)
      ...
      status('Skipping "' + d + '" in recovery mode; no .original file found.')
      return
  ...
  # If get_correct_name returns an empty string, the user has indicated that the
  # directory should be ignored. If we are in offline mode, it means that no
  # movie was found on IMDb.
  if n == "":
    if offlinemode:
...
def rename_directory(b, d, n):
  ...
  if cmp(old, new) == 0:
    ...
    errmsg('Cannot rename "' + d + '" to "' + n + '", directory already exists.')
  else:
    ...
    try:
      ...
      # Save original directory name, but only if there is not yet an .original
      # file.
...
def mkdir_and_move(b, f):
  ...
  if askmode and not confirm(
      prompt = 'Do you want to move the file "' + f + '" to the directory "' + n + '"? ',
      resp = True):
    return ""
  ...
  if not dirperm is None:
    change_permissions(d, dirperm)

  # Move the file. We are doing this using a system command, since there is no
  # good 'move' interface in python. (We cannot use shutils.move, since this
  # would first copy the source to the destination and then delete the source,
  # which takes a long time with movie files.)
  debug('Moving "' + f + '" to "' + n + '".')
...
def get_correct_name(b, d):
  ...
  # - If no .name file exists or if we are in force mode, we look up the movie
  #   using get_movie_for_directory(). 
  # - Otherwise, we are not in force mode and a .name file exists, so we can
  #   take the name from that file.
  # If get_movie_for_directory() returns an empty string, it means that no movie
  # could be determined for this directory, so we mark it as ignored in future
  # (unless we are in offline mode, in which case we need to add it to the list
  # of notifications). 
...
def get_movie_for_directory(b, d):
    if not forcemode and has_imdb_file(b, d):
      ...
      # We look up the movie on imdb according to its ID.
      # Because there is an .imdb file but no .name file, it is reasonable
      # to assume that the script was already run once and the user chose not to
      # give a custom name.
    ...
    # If a corresponding IMDb movie was found, then we set the .imdb file and
    # the rating file.
    if not m is None:
...
def add_title_attributes(d, s):
    ...
    # contained the words "unrated" or "director's cut" and if so then we add
    # the respective word to the title. 
...
def movie_by_name(s):
  ...
  # We give the user the opportunity to add a custom title, but not in offline
  # mode. 
  if not offlinemode:
  ...
  # If the user hasn't chosen a custom title but m is a valid movie object, we
  # take the name from it.
  if n == "" and not m is None:
    n = m.nice_title()

  # Before we return the movie, we fetch it again using movie_by_id() in order
  # to get the extended information such as the rating, unless speedy mode is
  # actived.
  if not m is None:
...
def imdb_search_movie_interactive(s):
  ...
  # The main loop keeps asking for user input, until the user has made a valid
  # choice, in which case we break from the loop or exit the function.
  while True:
    if len(results) == 0:
      print """
      No results found. Please enter a new string to search for, or just press
      enter to skip IMDb search.
      """
    else:
      print """
    ...
    # Just pressing return is a shortcut for selecting the first movie in the
    # list (if the list is nonempty).
    if len(results) > 0 and a == "":
      a = '1'

    # If the user enters 'i', or if no movie is found and he presses just enter,
    # then no movie is returned (i stands for "ignore").
    if a == "i" or (a == "" and len(results) == 0):
      return None

    # If the user entered a number, take the corresponding entry from the list.
    elif a.isdigit():
      n = int(a)
...
def ask_custom_title(m):
  while True:
    if m is None:
      ...
      print """
      Enter a custom title for this movie, or just enter if the name is fine.
      """
    ...
    if n != "":
      ...
      if m is None:
        sys.stdout.write(
            'You have chosen to ignore this directory. Please confirm ')
...
def imdb_query(n):
  ...
  if n.isdigit():
    ...
    r = api_search_movie(title)

  debug("Found %d possible movies." % len(r))

  # If there is a movie in r whose name is exactly n, then we move it to the top
  # of the list.
